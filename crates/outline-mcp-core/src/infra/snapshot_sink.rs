...
use ai_store_core::{Event, Seq, StoreError, StreamId};
use ai_store_sync::SyncProjectionSink;
...
impl SyncProjectionSink for SnapshotDumpSink {
    ...
    fn commit(
        ...
        if stream.as_str() != self.stream_key {
            // Not our stream (e.g. the per-node changelog) — never dump.
            return Ok(());
        }
...
mod tests {
    ...
    async fn test_commit_on_snapshot_stream_writes_file() {
        ...
        let seq = store
            ...
            .await
            .expect("append");
        let events = store.read(&stream, seq, 1).await.expect("read");
        let millis = events[0].at.0;
    ...
    async fn test_label_set_does_not_duplicate_body_write() {
        ...
        store
            .label_set(&stream, &Label::new("later-label"), seq)
            .await
            .expect("label_set");
