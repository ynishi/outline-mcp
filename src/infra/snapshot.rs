...
impl SnapshotService {
    ...
    pub fn tag(
        ...
        if !snapshot_path.exists() {
            return Err(
                format!("snapshot not found: {slug} at millis {timestamp_millis}").into(),
            );
        }
        let existing = Self::read_meta(shelf_dir, slug, timestamp_millis).unwrap_or_default();
        let created_at = existing
            .created_at
            .or(Some(Timestamp::now().as_millis()));
        let meta = SnapshotMeta {
            label: Some(label.to_string()),
            created_at,
        };
...
mod tests {
    ...
    fn test_tag_overwrites_existing_label() {
        ...
        let path =
            SnapshotService::create(&dir, "ow", &book, Some("initial")).expect("create");
        let stem = path.file_stem().unwrap().to_str().unwrap();
        let millis: i64 = stem.rsplit('.').next().unwrap().parse().unwrap();
